...
var data = {
  question1: 'portfolio job networking'.split(' '),
...
function createContent() {
  var question1, productType, color, size;
  var items = '';
  // dynamically create content
  for (var i=0, len1 = data.question1.length; i < len1; i++) {
    brand = data.question1[i];
    for (var j=0, len2 = data.productTypes.length; j < len2; j++) {
      productType = data.productTypes[j];
        ...
        for (var k=0, len4 = data.sizes.length; k < len4; k++) {
          size = data.sizes[k];
          var itemHtml = '<div class="item ' + question1 + ' ' +
